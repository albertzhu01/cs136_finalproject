...
#        sys.stdout.write("slot info: %s\n" % info)
    ...
    def expected_utils(self, t, history, reserve):
        ...
        """
        # Get clicks from previous round and slot info for each slot
        prev_round = history.round(t - 1)
        clicks = prev_round.clicks
        info = self.slot_info(t, history, reserve)
        utilities = []

        # Calculate payment as the reserve if bidder is the bottom-most allocated bidder.
        # Else, payment is the payment from the slot info, and utility is calculated using
    ...
    def bid(self, t, history, reserve):
        ...
        # Return bid according to conditions in the assignment
        if utilities[slot] <= 0:
            return self.value
        else:
            if slot > 0:
                return self.value - utilities[slot] / clicks[slot - 1]
            else:
                return self.value
